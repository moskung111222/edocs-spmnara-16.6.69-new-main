...
<head>
    ...
    <title>จัดการแบบฟอร์มดาวน์โหลด | สพม.นราธิวาส</title>
    <!-- Bootstrap 5 CSS -->
        ...
        <div class="row">
            ...
            <div class="col-md-3 col-lg-2 px-0 admin-sidebar d-flex flex-column">
                ...
                <div class="p-3 flex-grow-1">
                    <nav class="nav flex-column gap-2">
                        ...
                        <a href="<?= \App\Config\Config::SITE_URL ?>/admin/downloads" class="admin-sidebar-link active">
            ...
            <div class="col-md-9 col-lg-10 py-4 px-md-4">
                ...
                <div class="d-flex justify-content-between align-items-center mb-4 border-bottom pb-3 flex-wrap gap-2">
                    <div>
                        <h2 class="fw-bold mb-0 text-primary"><i class="fa-solid fa-download me-2"></i> จัดการแบบฟอร์มเปล่าในศูนย์ดาวน์โหลด</h2>
                        <p class="text-muted mb-0">แผงจัดการอัปโหลดเอกสารแบบฟอร์ม PDF / Word เพื่อให้ประชาชนดาวน์โหลดไปเขียนส่งก่อนยื่นคำขอ</p>
                    </div>
                    <div>
                        <button class="btn btn-premium px-4" data-bs-toggle="modal" data-bs-target="#uploadModal">
                            <i class="fa-solid fa-cloud-arrow-up me-2"></i> อัปโหลดแบบฟอร์มใหม่
                        </button>
                    </div>
                </div>
                        ...
                        <div class="table-responsive text-dark bg-white rounded-3 p-3">
                            <table class="table table-hover align-middle mb-0">
                                <thead class="table-light text-secondary">
                                    <tr>
                                        <th style="width: 35%;">หัวเรื่องแบบฟอร์มเอกสาร</th>
                                        <th style="width: 25%;">หมวดหมู่เอกสารยื่น</th>
                                        <th style="width: 15%;" class="text-center">ขนาดไฟล์</th>
                                        <th style="width: 15%;" class="text-center">วันที่ลงประกาศ</th>
                                    ...
                                    <?php if (!empty($items)): ?>
                                        <?php foreach ($items as $item): ?>
                                            <tr>
                                                <td>
                                                    <div class="d-flex align-items-center gap-2">
                                                        <?php 
                                                            ...
                                                            if ($ext === 'pdf') {
                                                                echo '<i class="fa-solid fa-file-pdf text-danger fs-5"></i>';
                                                            } else {
                                                                echo '<i class="fa-solid fa-file-word text-primary fs-5"></i>';
                                                            }
                                                        ?>
                                                        <div>
                                                ...
                                                <td>
                                                    <span class="badge bg-light text-teal border border-teal border-opacity-25 font-monospace small px-2.5 py-1">
                                                        <?= esc($categoryMapping[$item['category']] ?? $item['category']) ?>
                                                    </span>
                                                </td>
                                                <td class="text-center"><span class="small text-secondary"><?= esc(round($item['file_size'] / 1024, 2)) ?> KB</span></td>
                                                <td class="text-center"><span class="small text-secondary"><?= date('d/m/Y H:i', strtotime($item['created_at'])) ?> น.</span></td>
                                                <td class="text-center">
                                                    <div class="d-flex justify-content-center gap-1">
                                                        <a href="<?= \App\Config\Config::SITE_URL ?>/download?id=<?= $item['id'] ?>&source=download_center&download=1" class="btn btn-outline-success btn-sm rounded-pill px-2" title="ดาวน์โหลดไฟล์">
                                                            <i class="fa-solid fa-download"></i>
                                                        </a>
                                                        <form method="POST" class="d-inline" onsubmit="return confirm('ยืนยันที่จะลบไฟล์แบบฟอร์มออกจากระบบอย่างถาวรหรือไม่?');">
                                                            <?= \App\Middleware\CsrfMiddleware::getHtmlField() ?>
                                                            <input type="hidden" name="action" value="delete">
                                                            <input type="hidden" name="id" value="<?= $item['id'] ?>">
                                                            <button type="submit" class="btn btn-outline-danger btn-sm rounded-pill px-2" title="ลบแบบฟอร์ม">
                                    ...
                                    <?php else: ?>
                                        <tr>
                                            <td colspan="5" class="text-center py-5 text-muted">
                                                ...
                                                ยังไม่มีการอัปโหลดไฟล์แบบฟอร์มเอกสารเปล่าดาวน์โหลดในระบบ
                                            </td>
                                        </tr>
                                    <?php endif; ?>
                                </tbody>
            ...
            <div class="modal-content border-0 rounded-4 shadow">
                <form method="POST" enctype="multipart/form-data">
                    ...
                    <div class="modal-header bg-teal text-white border-0 py-3">
                        <h5 class="modal-title fw-bold text-white"><i class="fa-solid fa-cloud-arrow-up me-2"></i>อัปโหลดไฟล์แบบฟอร์มเปล่าใหม่</h5>
                        <button type="button" class="btn-close" data-bs-dismiss="modal" aria-label="Close"></button>
                    </div>
                    <div class="modal-body p-4">
                        <div class="mb-3 text-start">
                            <label class="form-label small fw-bold">หัวข้อแบบฟอร์มเอกสารเปล่า <span class="text-danger">*</span></label>
                            <input type="text" name="title" class="form-control" placeholder="เช่น แบบฟอร์มแผนการจัดการเรียนรู้ระดับมัธยมต้น" required>
                        </div>
                        <div class="mb-3 text-start">
                            <label class="form-label small fw-bold">หมวดหมู่เอกสารแบบฟอร์ม <span class="text-danger">*</span></label>
                            <select class="form-select" name="category" required>
                                <option value="Homeschool Application" selected>คำขอจัดการศึกษา (Homeschool)</option>
                                <option value="Education Plan Templates">แผนการจัดการเรียนรู้</option>
                                <option value="Learning Report Templates">แบบรายงานประเมินผลการเรียนรู้</option>
                                <option value="Transfer Requests">คำขอโอนย้ายสถานศึกษา</option>
                                <option value="Graduation Requests">คำขออนุมัติจบการศึกษา</option>
                                <option value="Other Documents">เอกสารแนวทางอื่นๆ</option>
                            </select>
                        </div>
                        <div class="mb-3 text-start">
                            <label class="form-label small fw-bold">เลือกไฟล์เอกสาร (PDF, DOC, DOCX ไม่เกิน 10MB) <span class="text-danger">*</span></label>
                            <input type="file" name="doc_file" accept=".pdf,.doc,.docx" class="form-control" required>
                        </div>
                    </div>
                    <div class="modal-footer border-0">
                        <button type="button" class="btn btn-secondary-premium px-4" data-bs-dismiss="modal">ยกเลิก</button>
                        <button type="submit" class="btn btn-premium px-4">ยืนยันอัปโหลด</button>
                    </div>
                </form>
            </div>
        </div>
